...
#
"""Speed-conditioned MaskedMimic student with Newton pressure-field feet enabled."""
...
def _load_base_experiment():
    module_path = Path(__file__).with_name("transformer_bm_speed.py")
    spec = spec_from_file_location("masked_mimic_transformer_bm_speed_base", module_path)
...
def additional_experiment_arguments(parser: argparse.ArgumentParser):
    """Reuse the speed-conditioned CLI and add pressure-field parameters."""
    _BASE_EXPERIMENT.additional_experiment_arguments(parser)
    parser.add_argument(
...
def configure_robot_and_simulator(
    ...
    """Reuse the student setup and enable foot-only pressure-field contact."""
