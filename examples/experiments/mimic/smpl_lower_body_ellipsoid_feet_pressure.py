...
#
"""Flat-ground Newton mimic experiment with pressure-field ellipsoid feet.
...
def additional_experiment_arguments(parser: argparse.ArgumentParser):
    """Expose the main pressure-field knobs for contact sweeps."""
    parser.add_argument(
        "--pressure-field-foot-kh",
        type=float,
        default=2.5e7,
        help="Hydroelastic stiffness for the SMPL lower-body foot shapes.",
    )
    parser.add_argument(
        "--pressure-field-foot-sdf-max-resolution",
        type=int,
        default=32,
        help="Sparse SDF resolution for the SMPL lower-body foot shapes.",
    )


def _validate_args(args: argparse.Namespace) -> None:
    if args.robot_name != "smpl_lower_body_ellipsoid_feet":
        raise ValueError(
            "This experiment requires --robot-name smpl_lower_body_ellipsoid_feet "
            "to keep the point-contact and pressure-field comparisons on the same foot geometry."
        )
...
def configure_robot_and_simulator(
    ...
    """Enable foot-only pressure-field contact against rigid flat terrain."""
    _validate_args(args)
