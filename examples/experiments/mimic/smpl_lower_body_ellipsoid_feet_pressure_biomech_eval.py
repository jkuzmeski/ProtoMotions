...
#
"""Biomechanics evaluation config for SMPL lower-body ellipsoid feet with pressure fields."""
...
def _load_base_experiment():
    module_path = Path(__file__).with_name("smpl_lower_body_ellipsoid_feet_pressure.py")
    spec = spec_from_file_location(
        "smpl_lower_body_ellipsoid_feet_pressure_base", module_path
    )
...
def additional_experiment_arguments(parser: argparse.ArgumentParser):
    """Expose pressure-field and evaluator-specific controls for analysis exports."""
    base_additional_arguments = getattr(
        _BASE_EXPERIMENT, "additional_experiment_arguments", None
    )
    if base_additional_arguments is not None:
        base_additional_arguments(parser)
