...
except ImportError:  # main does not always ship the fixed-speed controller
    SpeedControl = None
...
class BiomechanicsEvaluator(BaseEvaluator):
    ...
    def _get_speed_control_component(self) -> Any:
        ...
        is_speed_control = SpeedControl is not None and isinstance(component, SpeedControl)
        has_speed_interface = all(
            hasattr(component, attribute_name)
            for attribute_name in ("_tar_speed", "_tar_dir_theta", "_tar_dir")
        )
        if not (is_speed_control or isinstance(component, SteeringControl) or has_speed_interface):
            raise TypeError(
                "Expected a speed-compatible control component with target speed and "
                "direction state, "
    ...
    def _resolve_target_speeds(self) -> List[float]:
        ...
        if SpeedControl is not None and isinstance(control, SpeedControl):
            return [float(control.config.target_speed)]
        if hasattr(control.config, "target_speed"):
            return [float(control.config.target_speed)]
