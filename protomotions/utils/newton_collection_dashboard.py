...
def _run_git_optional(repo_root: pathlib.Path, args: list[str]) -> str | None:
    result = subprocess.run(
        ["git", *args],
        cwd=repo_root,
        check=False,
...
def _collect_branch(repo_root: pathlib.Path, branch_name: str) -> dict[str, Any]:
    ...
    commit = _run_git(repo_root, ["rev-parse", branch_name])
...
def _collect_run(run_dir: pathlib.Path, results_root: pathlib.Path) -> dict[str, Any]:
...
def collect_dashboard_data(repo_root: pathlib.Path, results_root: pathlib.Path) -> dict[str, Any]:
    ...
    runs = [_collect_run(run_dir, results_root) for run_dir in run_dirs if (run_dir / "run_config.json").exists()]
    ...
    git_state = _collect_git_state(repo_root, tracked_branch=tracked_branch, base_branch=base_branch)
    return {
...
def _render_run_details(run: dict[str, Any], is_latest: bool) -> str:
    ...
    iteration_chart_html = _render_iteration_chart(run)
...
def _render_iteration_chart(run: dict[str, Any]) -> str:
    ...
    summary_html = (
        ...
        f"{_render_metric_pair('metric', metric)}"
...
def render_dashboard_html(payload: dict[str, Any]) -> str:
    ...
    latest_run_html = (
        ...
        f"{_render_metric_pair('delta', _format_float(latest_run['improvement']))}"
        f"{_render_metric_pair('status', str(latest_run['latest_status']))}"
        "</div>"
        f'<p class="run-links">{_render_links(latest_run)}</p>'
        "</div>"
        if latest_run
        else '<div class="card"><p class="eyebrow">Latest Run</p><h2>No runs yet</h2></div>'
    )
...
<body>
  <main>
    <section class="hero">
      ...
      <h1>Branch health, run progress, and iteration evidence in one place.</h1>
