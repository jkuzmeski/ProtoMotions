...
#
"""Submit the S_GENERIC pressure-field student run through train_slurm.py."""
...
def _default_teacher_experiment_name(subset: str) -> str:
    return f"s_generic_teacher_{subset}_pressure"


def _default_student_experiment_name(subset: str) -> str:
    return f"s_generic_student_{subset}_pressure"
...
def create_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Submit the S_GENERIC pressure-field student job to Slurm. "
            "Defaults to scheduling the student for now+12hours."
        ),
    ...
    parser.add_argument("--teacher-experiment-name", default=None)
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Additional raw train_agent.py args appended after the expert-model "
            "and pressure-field defaults. This option must be last."
        ),
    )
...
def main() -> None:
    ...
    subset = args.subset
    teacher_experiment_name = args.teacher_experiment_name or _default_teacher_experiment_name(
        subset
    )
    experiment_name = args.experiment_name or _default_student_experiment_name(subset)
    remote_dir_name = args.remote_dir_name or _default_remote_dir_name(subset)
    expert_model_path = args.expert_model_path or f"results/{teacher_experiment_name}/last.ckpt"

    sync_paths = list(args.sync_paths)
    if args.sync_local_teacher_results:
        sync_paths.append(str(Path(expert_model_path).parent))
    ...
    cmd = [
        ...
        f"--slurm-begin={args.slurm_begin}",
    ...
    if sync_paths:
        cmd.extend(["--sync-paths", *sync_paths])
    if args.overrides:
        cmd.extend(["--overrides", *args.overrides])

    extra_args = [
        f"--expert-model-path={expert_model_path}",
