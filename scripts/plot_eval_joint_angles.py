...
def main() -> None:
    ...
    predicted_motion_lib = resolve_existing_path(args.predicted_motion_lib, base_dir=REPO_ROOT)
    run_dir = infer_run_dir(predicted_motion_lib)
    run_config = load_run_config(run_dir)

    reference_motion_source = (
        resolve_existing_path(args.reference_motion_source, base_dir=REPO_ROOT)
        if args.reference_motion_source is not None
        else infer_reference_motion_source(run_config)
    )
    model_xml = (
        resolve_existing_path(args.model_xml, base_dir=REPO_ROOT)
    ...
    output_dir = Path(args.output_dir).resolve() if args.output_dir is not None else default_output_dir
